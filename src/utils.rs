...
pub fn findChampionIndexFromID(champions : &VecDeque<SummonedChampion>, id : usize) -> Option<usize> { //(!D) swap this out for check targetable as well

	if champions[id].EqualId(id) { return Some(id) }
...
pub fn findChampionIndexFromIDTargetable(champions : &VecDeque<SummonedChampion>, id : usize) -> Option<usize> {
	let mut out : Option<usize> = None;
	if champions[id].EqualId(id) { out = Some(id) }
	else {
		for champ in champions { 
			if champ.EqualId(id) { 
				out = Some(id); 
